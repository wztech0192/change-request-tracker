  ...
  methods: {
    ...
    getDeleteTemplate(user) {
      var template = `${this.getUserInfo(user)}  
              ...
              <p>${user.role}</p>
              <hr>
              <label><i class='fa fa-exclamation-triangle'></i>&nbsp;&nbsp;Warning!</label>
              <h5>
              Are you sure you want to delete this user from the site? This process cannot be undone. </h5>
              <label><input class='acknowledge-ck' type='checkbox'> &nbsp;Action Acknowledged - Required</label>
              `;

      return template;
    },

    // get change role DOM template
